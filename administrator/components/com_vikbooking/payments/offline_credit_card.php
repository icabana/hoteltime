...
class vikBookingPayment {
	...
	static function getAdminParameters () {
		return array(
				'newstatus' => array('type' => 'select', 'label' => 'Set Reservation status to://if you want to manually verify that the credit card details are valid, set this to Pending', 'options' => array('CONFIRMED', 'PENDING')),
				...
				'cardtypes' => array('type' => 'text', 'label' => 'Credit Card Types://separate the choices with a comma, ex. Visa,Mastercard,Amex'),
	...
	public function __construct ($order, $params = array()) {
		...
		$this->order_info['oid'] = empty($this->order_info['id']) ? $this->order_info['details']['id'] : $this->order_info['id'];
	}
	...
	public function showPayment () {
		...
		//
		$cardtypes = array();
		if(!empty($this->params['cardtypes'])) {
			$all_cardtypes = explode(',', $this->params['cardtypes']);
		...
		$dbo = JFactory::getDBO();
		$q = "SELECT `paymentlog` FROM `#__vikbooking_orders` WHERE `id`=".(int)$this->order_info['oid'].";";
		$dbo->setQuery($q);
		$dbo->execute();
		if ($dbo->getNumRows() > 0) {
		...
		$form.='<div class="offline-cc-overlay-closer"><i class="fa fa-times"></i></div>'."\n";
		$form.="<p>".JText::_('VBCCOFFLINECCMESSAGE')."</p><form action=\"".$actionurl."\" method=\"post\" name=\"offlineccpaymform\">\n";
		$form.=$this->order_info['payment_info']['note']."<table>\n";
		if(count($cardtypes)) {
			$form.="<tr><td align=\"right\">".JText::_('VBCCCREDITCARDTYPE')." <sup>*</sup></td><td><select name=\"credit_card_type\">\n";
			foreach ($cardtypes as $cardtype) {
				$form.='<option value="'.$cardtype.'">'.$cardtype.'</option>'."\n";
			}
			$form.="</select></td></tr>\n";
		}
		$form.="<tr><td align=\"right\">".JText::_('VBCCCREDITCARDNUMBER')." <sup>*</sup></td><td><input type=\"text\" id=\"credit_card_number\" name=\"credit_card_number\" autocomplete=\"off\" size=\"20\" value=\"\"/></td></tr>\n";
		...
		$form.='</select></td></tr>'."\n";
		if($this->askcvv == 1) {
			$form.="<tr><td align=\"right\">".JText::_('VBCCCVV')." <sup>*</sup></td><td><input type=\"text\" id=\"credit_card_cvv\" name=\"credit_card_cvv\" autocomplete=\"off\" size=\"5\" value=\"\"/></td></tr>\n";
		}
		$form.="<tr><td align=\"right\">".JText::_('VBCCFIRSTNAME')." <sup>*</sup></td><td><input type=\"text\" id=\"business_first_name\" name=\"business_first_name\" autocomplete=\"off\" size=\"20\" value=\"\"/></td></tr>\n";
		$form.="<tr><td align=\"right\">".JText::_('VBCCLASTNAME')." <sup>*</sup></td><td><input type=\"text\" id=\"business_last_name\" name=\"business_last_name\" autocomplete=\"off\" size=\"20\" value=\"\"/></td></tr>\n";
		...
		$form.="</form>\n";
		$form.="</div></div>\n";
		$form.="<br clear=\"all\"/><button type=\"button\" class=\"btn\" id=\"offline-cc-overlay-toggler\">".JText::_('VBCCOFFLINECCTOGGLEFORM')."</button>\n";		
	...
	public function validatePayment () {
		...
		if(empty($creditcard) || strlen($creditcard) < 13 || strlen($creditcard) > 19 || (empty($cvv) && $this->askcvv == 1) || empty($business_first_name) || empty($business_last_name)) {
			$valid_data = false;
		}
		if(!empty($this->params['cardtypes']) && empty($creditcardtype)) {
			$valid_data = false;
		}
		if(!$valid_data) {
		...
		$mess = "Booking ID: ".$this->order_info['id']."\n\n";
	...
	public function afterValidation ($esit = 0) {
		...
		if($esit < 1) {
			VikError::raiseWarning('', JText::_('VBCCPAYMENTNOTVERIFIED'));
		}else {
