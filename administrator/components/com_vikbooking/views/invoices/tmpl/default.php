...
$pshow = VikRequest::getInt('show', '', 'request');
...
<form action="index.php?option=com_vikbooking&amp;task=invoices" method="post" name="invoicesform">
	<div id="filter-bar" class="btn-toolbar" style="width: 100%; display: inline-block;">
		...
		<div class="btn-group pull-left">
			<button type="button" class="btn btn-secondary" onclick="document.invoicesform.submit();"><i class="icon-search"></i></button>
			<button type="button" class="btn btn-secondary" onclick="document.getElementById('filterinvoices').value='';document.invoicesform.submit();"><i class="icon-remove"></i></button>
		</div>
	<?php
	if (!empty($rows)) {
	?>
			...
			<div class="vbo-invoices-inv-inner">
				...
				<div class="vbo-invoices-inv-back vbo-invoices-inv-face">
					...
					<div class="vbo-invoices-inv-back-commands">
						...
						<button type="button" class="btn" onclick="window.open('<?php echo VBO_SITE_URI; ?>helpers/invoices/generated/<?php echo $row['file_name']; ?>', '_blank');"><i class="vboicn-eye"></i><?php echo JText::_('VBOINVOPEN'); ?></button>
