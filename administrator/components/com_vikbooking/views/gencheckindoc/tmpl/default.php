...
<script type="text/javascript">
	...
	vbo_overlay_data['signature_link'] = '<div class="vbo-sign-share-meth-cont">'+
		...
		'</div>'+
		'<div class="vbo-sign-share-meth">'+
			'<span><a href="<?php echo JURI::root().'index.php?option=com_vikbooking&task=signature&sid='.$order['sid'].'&ts='.$order['ts']; ?>" target="_blank"><i class="vboicn-link"></i> <?php echo JURI::root().'index.php?option=com_vikbooking&task=signature&sid='.$order['sid'].'&ts='.$order['ts']; ?></a></span>'+
		'</div>'+
	'</div>';
	<?php
	if (!$empty_termsconds) {
		...
		?>
	vbo_overlay_data['termsconds'] = '<div><?php echo addslashes($termsconds); ?></div><div class="vbo-center"><br /><button type="button" class="btn btn-large btn-success" onclick="jQuery(\'#termsconds\').prop(\'checked\', true);vboCloseModal();"><?php echo addslashes(JText::_('VBOTERMSCONDSACCCLOSE')); ?></button></div>';
		<?php
	}
	if ($prefreshing > 0) {
		?>
...
<script type="text/javascript">
	...
	function vboConfirmGenerate(action) {
		if (action > 0) {
			...
			if (!document.getElementById('termsconds').checked) {
