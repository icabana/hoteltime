...
class VikBookingReportIstatSired extends VikBookingReport
{
	...
	public function getFilters()
	{
		...
		foreach ($this->map_regioni_prov as $region => $cities) {
			...
			foreach ($cities as $city) {
				$hidden_vals .= '		<option value="'.$region.'">'.$city.'</option>';
			}
		...
		$map_country_codes = array_merge(array_unique($this->map_country_codes), array('LIE' => 'Liechtenstein'));
	...
	public function customExport($export_type = 0)
	{
		...
		/**
		 * Hidden cols with empty values (we only have to skip 'Letti').
		 * The key will be used to place the hidden column after that key
		 * because 'Letti' should be after 'Camere', which is 'totrooms'
		 */
		$hidden_cols = array(
			'totrooms' => array(
				'label' => 'Letti',
		...
		foreach ($this->cols as $col) {
			...
			if (isset($col['export_name']) && !empty($col['export_name'])) {
				$col['label'] = $col['export_name'];
			}
			// push the column
			array_push($csvcols, $col['label']);
			// check if a hidden column should be set
			if (isset($hidden_cols[$col['key']])) {
				// push the hidden column
				array_push($csvcols, $hidden_cols[$col['key']]['label']);
			}
		}
		...
		foreach ($this->rows as $ind => $row) {
			...
			foreach ($row as $field) {
				...
				// get the value with the callback
				...
				// static check for Liechtenstein in case of 'tipo', which equals to 'Svizzera'
				if ($field['key'] == 'tipo' && $export_value == 'Liechtenstein') {
					$export_value = 'Svizzera';
				}

				// get the booking ID
				if ($field['key'] == 'idswh' && !in_array($export_value, $booking_ids)) {
					array_push($booking_ids, $export_value);
				}

				// push row value
				array_push($csvrow, $export_value);

				// check if a hidden value should be set
				if (isset($hidden_cols[$field['key']])) {
					// push hidden row value, usually an empty string
					array_push($csvrow, $hidden_cols[$field['key']]['value']);
				}
			}
		...
		header('Content-Disposition: attachment; filename="'.str_replace(' ', '_', $this->reportName).'-'.str_replace('/', '_', $pfromdate).'-'.str_replace('/', '_', $ptodate).'.csv"');
		$outstream = fopen("php://output", 'w');
		foreach ($csvlines as $csvline) {
			// delimiter must be a semi-colon
