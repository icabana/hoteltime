...
$dbo = JFactory::getDBO();
$vbo_tn = modVikbooking_horizontalsearchHelper::getTranslator();
...
jQuery(function(){
	...
	jQuery('.vb-cal-img').click(function(){
		...
		?>
		<div class="vbmodhorsearchcheckindiv vbmodhorsearch_datefield"><label for="checkindatemod<?php echo $randid; ?>"><?php echo JText :: _('VBMCHECKIN'); ?>:</label><div class="input-group"><input type="text" name="checkindate" id="checkindatemod<?php echo $randid; ?>" size="10"/><i class="fa fa-calendar"></i><input type="hidden" name="checkinh" value="<?php echo $hcheckin; ?>"/><input type="hidden" name="checkinm" value="<?php echo $mcheckin; ?>"/></div></div>
		<div class="vbmodhorsearchcheckoutdiv vbmodhorsearch_datefield"><label for="checkoutdatemod<?php echo $randid; ?>"><?php echo JText :: _('VBMCHECKOUT'); ?>:</label><div class="input-group"><input type="text" name="checkoutdate" id="checkoutdatemod<?php echo $randid; ?>" size="10"/><i class="fa fa-calendar"></i><input type="hidden" name="checkouth" value="<?php echo $hcheckout; ?>"/><input type="hidden" name="checkoutm" value="<?php echo $mcheckout; ?>"/></div></div>
		<?php
	}else {
		//default Joomla Calendar
		JHTML::_('behavior.calendar');
		?>
		<div class="vbmodhorsearchcheckindiv vbmodhorsearch_datefield"><label for="checkindatemod<?php echo $randid; ?>"><?php echo JText :: _('VBMCHECKIN'); ?>:</label><?php echo JHTML :: _('calendar', '', 'checkindate', 'checkindatemod'.$randid, $vbdateformat, array ('class' => '','size' => '10','maxlength' => '19')); ?><input type="hidden" name="checkinh" value="<?php echo $hcheckin; ?>"/><input type="hidden" name="checkinm" value="<?php echo $mcheckin; ?>"/></div>
		<div class="vbmodhorsearchcheckoutdiv vbmodhorsearch_datefield"><label for="checkoutdatemod<?php echo $randid; ?>"><?php echo JText :: _('VBMCHECKOUT'); ?>:</label><?php JHTML :: _('calendar', '', 'checkoutdate', 'checkoutdatemod'.$randid, $vbdateformat, array ('class' => '','size' => '10','maxlength' => '19')) ?><input type="hidden" name="checkouth" value="<?php echo $hcheckout; ?>"/><input type="hidden" name="checkoutm" value="<?php echo $mcheckout; ?>"/></div>
		<?php
	}
	//
	...
	if (intval($maxsearchnumrooms) > 1) {
		$roomsel = "<div class=\"vbmodhorsearch_selfield\"><span class=\"vbhsrnselsp\"><select name=\"roomsnum\" id=\"vbmodformroomsn".$randid."\" onchange=\"vbSetRoomsAdultsMod".$randid."(this.value);\">\n";
		for($r = 1; $r <= intval($maxsearchnumrooms); $r++) {
			$roomsel .= "<option value=\"".$r."\">".$r."</option>\n";
		}
		$roomsel .= "</select></span><i class=\"fa fa-home\"></i></div>\n";
	}else {
	...
	<div class="vbmodhorsearchrac">
		...
		<div class="vbmodhorsearchroomsel"><?php if(intval($maxsearchnumrooms) > 1): ?><label for="vbmodformroomsn<?php echo $randid; ?>"><?php echo JText::_('VBMFORMROOMSN'); ?>:</label><?php endif; ?><?php echo $roomsel; ?></div>
		...
		<div class="vbmodhorsearchroomdentr">
			...
			<div class="vbmodhorsearchroomdentrfirst">
				...
				<div class="horsanumdiv"><label class="horsanumlb"><?php echo JText::_('VBMFORMADULTS'); ?></label><div class="vbmodhorsearch_selfield"><span class="horsanumsel"><?php echo $adultsel; ?></span><i class="fa fa-user"></i></div></div>
				<?php if($showchildren): ?><div class="horscnumdiv"><label class="horscnumlb"><?php echo JText::_('VBFORMCHILDREN'); ?></label><div class="vbmodhorsearch_selfield"><span class="horscnumsel"><?php echo $childrensel; ?></span><i class="fa fa-child"></i></div></div><?php endif; ?>
			</div>
	...
	if (intval($params->get('showcat')) == 1) {
		...
		<div class="vbmodhorsearchcategoriesblock">
			...
			<div class="vbmodhorsearch_selfield">
		...
		$q = "SELECT * FROM `#__vikbooking_categories` WHERE `id`=".(int)$params->get('category_id').";";
		$dbo->setQuery($q);
		$dbo->Query($q);
		if ($dbo->getNumRows() > 0) {
			$categories = $dbo->loadAssocList();
	...
	?>
		...
		<div class="vbmodhorsearchbookdiv">
			<input type="submit" name="search" value="<?php echo (strlen($params->get('srchbtntext')) > 0 ? $params->get('srchbtntext') : JText::_('SEARCHD')); ?>" class="btn vbsearchinputmodhors"/>
		</div>
	...
	function vbAddElementMod<?php echo $randid; ?>() {
		...
		newdiv.innerHTML = '<div class=\'vbmodhorsearchroomdentr\'><span class=\'horsrnum\'><?php echo JText::_('VBMFORMNUMROOM'); ?> '+ num +'</span><div class=\'horsanumdiv\'><div class=\"vbmodhorsearch_selfield\"><span class=\'horsanumsel\'><?php echo addslashes(str_replace('"', "'", $adultsel)); ?></span><i class=\"fa fa-user\"></i></div><?php echo ($showchildren ? "<div class=\'horscnumdiv vbmodhorsearch_selfield\'><span class=\'horscnumsel\'>".addslashes(str_replace('"', "'", $childrensel))."</span><i class=\"fa fa-child\"></i></div" : ""); ?></div>';
