...
<div class="vbo-results-head vbo-results-head-showprc">
	<span class="vbo-results-nights"><i class="fa fa-calendar"></i> <span><?php echo $days; ?> <?php echo ($days == 1 ? JText::_('VBSEARCHRESNIGHT') : JText::_('VBSEARCHRESNIGHTS')); ?></span></span>
<?php
if($roomsnum > 1) {
	?>
	<span class="vbo-results-numrooms"><i class="fa fa-key"></i> <span><?php echo $roomsnum." ".($roomsnum == 1 ? JText::_('VBSEARCHRESROOM') : JText::_('VBSEARCHRESROOMS')); ?></span></span>
	<?php
}
?>
	<span class="vbo-results-numadults"><i class="fa fa-users"></i> <span><?php echo $totadults; ?> <?php echo ($totadults == 1 ? JText::_('VBSEARCHRESADULT') : JText::_('VBSEARCHRESADULTS')); ?></span></span>
<?php
if($showchildren && $totchildren > 0) {
	?>
	<span class="vbo-results-numchildren"><i class="fa fa-child"></i> <span><?php echo $totchildren." ".($totchildren == 1 ? JText::_('VBSEARCHRESCHILD') : JText::_('VBSEARCHRESCHILDREN')); ?></span></span>
	<?php
}
?>
</div>
...
<div class="vbsearchresheadcheckroom">
	<div class="vbcheckinroom"><i class="fa fa-sign-in"></i> <span><?php echo JText::_('VBCHECKINONTHE'); ?> <?php echo JText::sprintf('VBCHECKINOUTOF', vikbooking::sayDayMonth($infocheckin['mday']), vikbooking::sayMonth($infocheckin['mon'])); ?></span></div>
	<div class="vbcheckoutroom"><i class="fa fa-sign-out"></i> <span><?php echo JText::_('VBCHECKOUTONTHE'); ?> <?php echo JText::sprintf('VBCHECKINOUTOF', vikbooking::sayDayMonth($infocheckout['mday']), vikbooking::sayMonth($infocheckout['mon'])); ?></span></div>
</div>
	...
	?>
		<div class="vbo-showprc-room-block<?php echo $roomsnum > 1 ? ' vbo-showprc-room-block-multi' : ''; ?>">
			...
			<div class="vbo-showprc-staydetails">
				<div class="vbo-showprc-staydetails-party"><i class="fa fa-users"></i> <span><?php echo $arrpeople[$num]['adults']; ?> <?php echo ($arrpeople[$num]['adults'] == 1 ? JText::_('VBSEARCHRESADULT') : JText::_('VBSEARCHRESADULTS')); ?><?php echo ($showchildren && $arrpeople[$num]['children'] > 0 ? ", ".$arrpeople[$num]['children']." ".($arrpeople[$num]['children'] == 1 ? JText::_('VBSEARCHRESCHILD') : JText::_('VBSEARCHRESCHILDREN')) : ""); ?></span></div>
				<div class="vbo-showprc-staydetails-nights"><i class="fa fa-calendar"></i> <span><?php echo (intval($tars[$num][0]['days']) == 1 ? "1 ".JText::_('VBDAY') : $tars[$num][0]['days']." ".JText::_('VBDAYS')); ?></span></div>
			</div>
		...
		?>
				...
				<div class="vbo-showprc-info-room">
					<div class="vbo-showprc-roomname other-font"><?php echo $r['name']; ?></div>
			...
			?>
						<div class="vbo-showprc-price-entry">
							<div class="vbo-showprc-price-entry-radio">
								<input type="radio" class="vbo-radio" data-roomnum="<?php echo $num; ?>" data-ratecost="<?php echo $rplan_cost; ?>" name="priceid<?php echo $num; ?>" id="pid<?php echo $num.$t['idprice']; ?>" value="<?php echo $t['idprice']; ?>"<?php echo ($k==0 ? " checked=\"checked\"" : ""); ?>/>
							</div>
		...
		if($is_package === true) {
			...
			?>
						<div class="vbo-showprc-price-entry vbo-showprc-price-pkg">
							<div class="vbo-showprc-price-entry-radio">
								<input type="radio" class="vbo-radio" data-roomnum="<?php echo $num; ?>" data-ratecost="<?php echo $base_pkg_cost; ?>" name="priceid<?php echo $num; ?>" id="pid<?php echo $num.$pkg['id']; ?>" value="<?php echo $pkg['id']; ?>" checked="checked"/>
							</div>
							<div class="vbo-showprc-price-entry-rateplan">
		...
		if (!empty($r['idopt']) && is_array($optionals)) {
			...
			if (is_array($ageintervals) && count($ageintervals) > 0 && $arrpeople[$num]['children'] > 0 && $skip_all_opt !== true) {
				...
				foreach($intervals as $kintv => $intv) {
					...
					$chageselect .= '<option value="'.($kintv + 1).'"'.($pcent_interval === true ? ' data-pcentintvroom="'.$num.'" data-pcentintv="'.floatval($intvparts[2]).'" data-ageintv="'.$intvparts[0].' - '.$intvparts[1].'"' : '').'>'.$intvparts[0].' - '.$intvparts[1].' ('.$pricestr.' '.$currencysymb.')'.'</option>'."\n";
				}
				$chageselect .= '</select>'."\n";
				?>
				...
				for($ch = 1; $ch <= $arrpeople[$num]['children']; $ch++) {
					?>
							<li><div class="vbo-sel"><span class="vbo-sel-lb"><?php echo JText::_('VBSEARCHRESCHILD').' #'.$ch; ?>:</span> <span><?php echo $chageselect; ?></span><i class="fa fa-sort"></i></div></li>
					<?php
				}
				?>
						</ul>
			...
			if(intval($o['forcesel']) == 1) {
				...
				if(intval($o['hmany']) == 1) {
					if(intval($o['maxquant']) > 0) {
						$optquaninp = "<div class=\"vbo-sel\"><span><select name=\"optid".$num.$o['id']."\">\n";
						for($ojj = 0; $ojj <= intval($o['maxquant']); $ojj++) {
							$optquaninp .= "<option value=\"".$ojj."\">".$ojj."</option>\n";
						}
						$optquaninp .= "</select></span><i class=\"fa fa-sort\"></i></div>\n";
					}else {
...
foreach($arrpeople as $indroom => $aduch) {
	...
	<div class="room_buttons_box">
		...
		if($is_package === true) {
			?>
			<a class="btn btn-grey" href="<?php echo JRoute::_('index.php?option=com_vikbooking&view=packagedetails&pkgid='.$pkg['id'], false); ?>"><?php echo JText::_('VBBACK'); ?></a>
			<?php
		}else {
			?>
			<a class="btn btn-grey" href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&checkindate='.urlencode($checkinforlink).'&checkoutdate='.urlencode($checkoutforlink).'&roomsnum='.$roomsnum.$peopleforlink, false); ?>"><?php echo JText::_('VBBACK'); ?></a>
			<?php
		}
		?>
		</div>
		...
		if(!isNaN(rcost) && jQuery("option[data-pcentintvroom='"+rnum+"']").length > 0) {
			jQuery("option[data-pcentintvroom='"+rnum+"']").each(function() {
				...
				if(!isNaN(pcentval) && agesval.length) {
					var intvcost = rcost * pcentval / 100;
