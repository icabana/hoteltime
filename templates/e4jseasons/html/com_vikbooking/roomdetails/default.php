...
<div class="vblistroomnamediv">
	<span class="vblistroomname other-font"><?php echo $room['name']; ?></span>
	<span class="vblistroomcat"><span><?php echo VikBooking::sayCategory($room['idcat'], $vbo_tn); ?></span></span>
</div>
...
<div class="vbo-availcalendars-cont">
	...
	<form action="<?php echo JRoute::_('index.php?option=com_vikbooking&view=roomdetails&roomid='.$room['id'].'&Itemid='.$pitemid); ?>" method="post" name="vbmonths">
		<div class="vbo-sel"><span><select name="month" onchange="javascript: document.vbmonths.submit();" class="vbselectm"><?php echo $moptions; ?></select></span><i class="fa fa-sort"></i></div>
...
function vboCheckClosingDates(date) {
	var isdayopen = vbIsDayOpen(date);
	return [isdayopen];
}
...
jQuery(function(){
	...
	jQuery('.vb-cal-img, .vbo-caltrigger').click(function(){
		...
		$selform .= "<div class=\"vbo-search-inpblock vbo-search-inpblock-checkout\"><label for=\"checkoutdate\">" . JText::_('VBRETURNROOM') . "</label><div class=\"input-group\"><input type=\"text\" name=\"checkoutdate\" id=\"checkoutdate\" size=\"10\" autocomplete=\"off\"/><i class=\"fa fa-calendar vbo-caltrigger\"></i></div><input type=\"hidden\" name=\"checkouth\" value=\"".$hcheckout."\"/><input type=\"hidden\" name=\"checkoutm\" value=\"".$mcheckout."\"/></div>\n";
	} else {
	...
	if($multi_units === 1 && $room['units'] > 1) {
		...
		$roomsel = "<label for=\"vbo-detroomsnum\">".JText::_('VBFORMROOMSN')."</label><div class=\"vbo-sel\"><span><select id=\"vbo-detroomsnum\" name=\"roomsnum\" onchange=\"vbSetRoomsAdults(this.value);\">\n";
		for($r = 1; $r <= $maxsearchnumrooms; $r++) {
			$roomsel .= "<option value=\"".$r."\">".$r."</option>\n";
		}
		$roomsel .= "</select></span><i class=\"fa fa-sort\"></i></div>\n";
	}else {
	...
	$selform .= "			<div class=\"vbo-search-num-adults-entry\"><label class=\"vbo-search-num-adults-entry-label\">".JText::_('VBFORMADULTS')."</label><div class=\"vbo-sel\"><span class=\"vbo-search-num-adults-entry-inp\">".$adultsel."</span><i class=\"fa fa-sort\"></i></div></div>\n";
	if($showchildren) {
		$selform .= "		<div class=\"vbo-search-num-children-entry\"><label class=\"vbo-search-num-children-entry-label\">".JText::_('VBFORMCHILDREN')."</label><div class=\"vbo-sel\"><span class=\"vbo-search-num-children-entry-inp\">".$childrensel."</span><i class=\"fa fa-sort\"></i></div></div>\n";
	}
	$selform .= "		</div>\n";
	$selform .= "	</div>\n";
	...
	function vbAddElement() {
		...
		newdiv.innerHTML = '<div class=\'vbo-search-num-aduchild-entry\'><span class=\'vbo-search-roomnum\'><?php echo addslashes(JText::_('VBFORMNUMROOM')); ?> '+ num +'</span><div class=\'vbo-search-num-adults-entry\'><label class=\'vbo-search-num-adults-entry-label\'><?php echo addslashes(JText::_('VBFORMADULTS')); ?></label><div class=\"vbo-sel\"><span class=\'vbo-search-num-adults-entry-inp\'><?php echo addslashes(str_replace('"', "'", $adultsel)); ?></span><i class=\"fa fa-sort\"></i></div></div><?php if($showchildren): ?><div class=\'vbo-search-num-children-entry\'><label class=\'vbo-search-num-children-entry-label\'><?php echo addslashes(JText::_('VBFORMCHILDREN')); ?></label><div class=\"vbo-sel\"><span class=\'vbo-search-num-adults-entry-inp\'><?php echo addslashes(str_replace('"', "'", $childrensel)); ?></span><i class=\"fa fa-sort\"></i></div></div><?php endif; ?></div>';
