...
<?php if (isset($images->image_intro) && !empty($images->image_intro)) : ?>
	...
	<div class="thumbnail-div pull-<?php echo htmlspecialchars($imgfloat, ENT_COMPAT, 'UTF-8'); ?> item-image"> 
	<?php if ($params->get('link_titles') && $params->get('access-view')) { ?>
		<a href="<?php echo JRoute::_(ContentHelperRoute::getArticleRoute($displayData->slug, $displayData->catid, $displayData->language)); ?>">
			...
			<?php echo 'class="thumbnail caption"' . ' title="' . htmlspecialchars($images->image_intro_caption) . '"'; 
			} else { 
	...
	<?php } else { ?><img
		<?php if ($images->image_intro_caption) {
			echo 'class="thumnail caption"' . ' title="' . htmlspecialchars($images->image_intro_caption, ENT_COMPAT, 'UTF-8') . '"';
		} else { 
			echo 'class="thumbnail"'; 
		} ?>	
