...
class VikBookingCustomersPin {
	...
	*/
	public function getCustomerByID($cust_id) {
		$customer = array();
		if (!empty($cust_id)) {
			$q = "SELECT * FROM `#__vikbooking_customers` WHERE `id`=".$this->dbo->quote($cust_id).";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() == 1) {
				$customer = $this->dbo->loadAssoc();
	...
	*/
	public function getCustomerFromBooking($orderid) {
		$customer_id = 0;
		if (!empty($orderid)) {
			$q = "SELECT `idcustomer` FROM `#__vikbooking_customers_orders` WHERE `idorder`=".(int)$orderid.";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() == 1) {
	...
	*/
	...
	/**
	* Since VBO 1.9: calculates and sets the commissions amount
	* if the customer is a sales channel with a percentage greater than 0
	* @param orderid
	* @param customer_id
	*/
					...
					if (array_key_exists('applycmmon', $customer['chdata']) && (int)$customer['chdata']['applycmmon'] > 0) {
						...
						if ($this->pricesTaxIncluded()) {
							...
							foreach ($map_cost_tax as $ctk => $ctv) {
								...
								$aliq = $this->getAliqFromTaxId($ctv['taxid']);
								if ((float)$aliq > 0.00) {
									$op_div = (100 + $aliq) / 100;
									$map_cost_tax[$ctk]['amount'] = $ctv['amount'] / $op_div;
								}
							}
						}
					}
	...
	*/
	private function getAliqFromTaxId($taxid) {
		$aliq = 0;
		if (intval($taxid) > 0) {
			$q = "SELECT `i`.`aliq` FROM `#__vikbooking_iva` AS `i` WHERE `i`.`id`=".(int)$taxid.";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() == 1) {
	...
	*/
	public function pluginCustomerSync($customer_id, $mode) {
		...
		$q = "SELECT * FROM `#__vikbooking_customers` WHERE `id`=".(int)$customer_id." LIMIT 1;";
		$this->dbo->setQuery($q);
		$this->dbo->execute();
		if ($this->dbo->getNumRows() > 0) {
			$customer = $this->dbo->loadAssoc();
