...
class VikbookingViewRoomdetails extends JViewVikBooking {
	function display($tpl = null) {
		...
		if($dbo->getNumRows() == 1) {
			...
			//
			// VBO 1.10 - attempt to load the "terms and conditions" checkbox
			$terms_fields = array();
			$q = "SELECT * FROM `#__vikbooking_custfields` WHERE `type`='checkbox' AND `required`=1 ORDER BY `#__vikbooking_custfields`.`ordering` DESC LIMIT 1;";
			$dbo->setQuery($q);
			$dbo->execute();
			if ($dbo->getNumRows() > 0) {
