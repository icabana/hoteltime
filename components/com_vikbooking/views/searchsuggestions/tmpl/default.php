...
if (count($this->suggestions)) {
	...
	?>
<div class="vbo-search-suggestions-container">
		...
		<table class="table">
			...
			foreach ($sug_calendars as $calkey => $caldays) {
				...
				<tr class="vbo-search-suggestions-month-days">
					<td class="vbo-search-suggestions-av-mon" rowspan="2"><?php echo $map_months[((int)$monparts[0] - 1)].' '.$monparts[1]; ?></td>
				<?php
				$days_counter = 0;
		...
		foreach ($this->solutions as $keyday => $solution) {
			...
			<div class="vbo-search-solution" data-daysol="<?php echo implode(';', $nights_suggested); ?>">
				...
				<div class="vbo-search-solution-book vbselectr">
					<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&checkindate='.date($df, $day_info[0]).'&checkinh='.$hcheckin.'&checkinm='.$mcheckin.'&checkoutdate='.date($df, $out_info[0]).'&checkouth='.$hcheckout.'&checkoutm='.$mcheckout.'&roomsnum='.count($this->party).$aduchild_str.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>"><?php echo JText::_('VBOBOOKSOLSEARCHROOMS'); ?></a>
				</div>
			</div>
			<?php
			$sug_count++;
		...
		foreach ($this->booking_solutions as $keyday => $solution) {
			...
			<div class="vbo-booking-solution" data-daysol="<?php echo implode(';', $nights_suggested); ?>">
				...
				<div class="vbo-booking-solution-book vbselectr">
					<a href="<?php echo JRoute::_('index.php?option=com_vikbooking&task=search&checkindate='.date($df, $day_info[0]).'&checkinh='.$hcheckin.'&checkinm='.$mcheckin.'&checkoutdate='.date($df, $out_info[0]).'&checkouth='.$hcheckout.'&checkoutm='.$mcheckout.'&roomsnum='.$sol_tot_rooms.$aduchild_str.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>"><?php echo JText::_('VBBOOKNOW'); ?></a>
				</div>
			</div>
			<?php
			$sug_count++;
...
jQuery(".vbo-search-solution, .vbo-booking-solution").hover(function() {
	var daysol = jQuery(this).attr("data-daysol").split(";");
	if (!daysol.length) {
		return;
	}
	for (var i = 0; i < daysol.length; i++) {
		var avday = jQuery("td.vbo-search-suggestions-month-day[data-day='"+daysol[i]+"']");
		if (avday && avday.length) {
			avday.addClass("vbo-search-suggestions-month-day-highlighted");
		}
	}
}, function() {
	jQuery("td.vbo-search-suggestions-month-day-highlighted").removeClass("vbo-search-suggestions-month-day-highlighted");
});
...
var vbo_suggestions = <?php echo json_encode($this->suggestions); ?>;
jQuery(".vbo-suggestion-free").click(function() {
	...
	if (vbo_suggestions.hasOwnProperty(keyday)) {
		jQuery(".vbo-suggestion-free.vbo-suggestion-selected").removeClass("vbo-suggestion-selected");
			...
			?>
				...
				if (paramshowpeople > 0) {
					...
					if (paramshowpeople == 1) {
						daycont += '<div class="vbmaxadultsdet"><span class="vbmaximgdet"><i class="fa fa-male"></i></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMADULTS')); ?></span><span class="vbmaxnumberdet">'+maxadustr+'</span></div>';
					} else if (paramshowpeople == 2) {
						daycont += '<div class="vbmaxchildrendet"><span class="vbmaximgdet"><i class="fa fa-child"></i></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMCHILDREN')); ?></span><span class="vbmaxnumberdet">'+maxchistr+'</span></div>';
					} else if (paramshowpeople == 3) {
						daycont += '<div class="vbmaxadultsdet"><span class="vbmaximgdet"><i class="fa fa-male"></i></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBFORMADULTS')); ?></span><span class="vbmaxnumberdet">'+maxadustr+'</span></div>';
						daycont += '<div class="vbmaxtotdet"><span class="vbmaximgdet"><i class="fa fa-users"></i></span><span class="vbmaxlabeldet"><?php echo addslashes(JText::_('VBMAXTOTPEOPLE')); ?></span><span class="vbmaxnumberdet">'+maxtotstr+'</span></div>';
