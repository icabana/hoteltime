				...
				if (VikBooking::dateIsValid($pcheckindate) && VikBooking::dateIsValid($pcheckoutdate)) {
					...
					if ($second > $first && $first >= $actnow) {
						...
						if ($restrictionsvalid === true) {
							...
							$dbo = JFactory::getDBO();
							$q = "SELECT `p`.*,`r`.`id` AS `r_reference_id`,`r`.`name`,`r`.`img`,`r`.`idcat`,`r`.`idcarat`,`r`.`units`,`r`.`fromadult`,`r`.`toadult`,`r`.`fromchild`,`r`.`tochild`,`r`.`smalldesc`,`r`.`totpeople`,`r`.`params`,`rp`.`minlos`,`rp`.`minhadv` FROM `#__vikbooking_dispcost` AS `p`, `#__vikbooking_rooms` AS `r`, `#__vikbooking_prices` AS `rp` WHERE `p`.`days`=".(int)$daysdiff." AND `p`.`idroom`=`r`.`id` AND `p`.`idprice`=`rp`.`id` AND `r`.`avail`='1' AND (".implode(" OR ", $arradultsclause).") ORDER BY `p`.`cost` ASC, `p`.`idroom` ASC;";
							$dbo->setQuery($q);
							$dbo->execute();
							if ($dbo->getNumRows() > 0) {
								...
								if (@count($arrtar) > 0) {
									...
									if (count($results) == $proomsnum) {
										...
										$proomdetail = VikRequest::getInt('roomdetail', '', 'request');
										...
										if (!empty($proomdetail) && array_key_exists($proomdetail, $arrtar) && $proomsnum == 1) {
											...
											if (!empty($proomdetail) && $proomsnum == 1) {
												...
												if ($dbo->getNumRows() > 0) {
													...
													VikError::raiseWarning('', JText::sprintf('VBDETAILCNOTAVAIL', $cdet[0]['name'], $daysdiff));
												}
												...
												if ($room_missing === false) {
													...
													if ($dbo->getNumRows() > 0) {
														$cdet=$dbo->loadAssocList();
														$vbo_tn->translateContents($cdet, '#__vikbooking_rooms');
														VikError::raiseWarning('', JText::sprintf('VBDETAILMULTIRNOTAVAIL', $proomsnum, $cdet[0]['name'], $daysdiff));
													}
												}
											}
											$this->res = &$results;
										...
										$this->setVboError(JText::sprintf('VBSEARCHERRNOTENOUGHROOMS', implode(" - ", $errmess)), $err_code_info);
									}
								} else {
									$err_code_info = array();
									if (strlen($restrictionerrmsg) > 0) {
									...
									$this->setVboError(JText::_('VBNOROOMSINDATE'), $err_code_info);
								}
							} else {
